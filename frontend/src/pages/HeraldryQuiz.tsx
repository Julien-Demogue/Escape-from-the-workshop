...
function computeScore(ms:number){ for(const r of SCORING_TABLE){ if(ms<=r.maxMs) return r.score } return FALLBACK_SCORE }
function formatDuration(ms:number){ const s=Math.floor(ms/1000); const m=Math.floor(s/60); const r=s%60; return `${String(m).padStart(2,"0")}:${String(r).padStart(2,"0")}` }
...
export default function HeraldryQuiz() {
  ...
  function finishSuccess(){
    ...
    setRunning(false); setElapsed(elapsedMs); setSolvedThisSession(true);
    const newScore = computeScore(elapsedMs); setScore(newScore);
    reportGameResult("heraldry-quiz",{ status:"completed", score:newScore, codePart: codePartFor("heraldry-quiz") });
  }
  function finishFail(){ setRunning(false); reportGameResult("heraldry-quiz",{ status:"failed" }); }
  ...
  function onPick(visibleIndex:number){
    ...
    setAnswers(prev => { const n=[...prev]; n[step]=visibleIndex; return n; });
    setCorrectFlags(prev => { const n=[...prev]; n[step]=isCorrect; return n; });

    setTimeout(()=>{
      if(step<QUESTIONS.length-1){ setStep(s=>s+1); setLocked(false); }
      else {
  ...
  function hardReset(){
    setStep(0); setAnswers([]); setCorrectFlags([]); setRunId(x=>x+1); setLocked(false);
    startRef.current = Date.now(); setElapsed(0); setRunning(true); setSolvedThisSession(false);
  }
  ...
  return (
    <div className="hq-shell">
      ...
      <div className="hq-card">
        <h1 className="hq-title">Devine le blason</h1>
